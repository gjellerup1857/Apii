...
本文件說明如何將 `Apii` 推送到 GitHub 並完成開源發布。

## 1. 在 GitHub 上建立空倉庫

1. 登入 GitHub → https://github.com/new
2. **Repository name**：`Apii`（或自訂）
...
本專案已初始化為獨立 git repo（`Apii/.git`），並完成首次提交 `f489e9b`。

在 `Apii` 目錄下執行：

```bash
cd "/Users/user/Documents/Default Project/Apii"

# 1. 替換為你的 GitHub 倉庫 URL（以下用 SSH 為例，HTTPS 亦可）
git remote add origin git@github.com:<your-username>/Apii.git
# 或 HTTPS
# git remote add origin https://github.com/<your-username>/Apii.git
...
- **自動**：推送 `v*` 標籤後，`.github/workflows/release.yml` 會自動打包 `dist/Apii-v*.zip` 並建立 GitHub Release。
- **手動**：GitHub → Releases → Draft a new release → 選擇 `v1.0.2` → 上傳 `dist/Apii-v1.0.2.zip`（本地 `npm run build:release` 產生）→ 發布。
...
```bash
npm run lint          # 語法檢查
npm run build         # 打包 dist/Apii.zip
...
完成後，你的開源地址將為 `https://github.com/<your-username>/Apii`。
