...
### Changed
- 初始高度再拉高 240px：`720 → 960`（`popup` 與 `floating` 同步，`max-height:90vh` 限制）


## [1.0.7] - 2026-08-31

### Added
- 展開的新標籤頁可被加入 Brave 側邊欄：`fullpage.html` 新增「加入側邊欄」按鈕（`chrome.sidePanel.setOptions/open` + 複製網址備援），RWD 已適配 320px 窄版，提示可常駐
